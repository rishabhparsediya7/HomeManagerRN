...
const ListHeaderComponent = ({ navigation, firstName, lastName, image, profileImage, styles }: any) => {
    return (
    ...
    return (
        <SafeAreaView style={styles.container} edges={['left', 'right']}>
            <KeyboardAvoidingView
                ...
                <ListHeaderComponent navigation={navigation} firstName={firstName} lastName={lastName} image={image} profileImage={profileImage} styles={styles} />
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={[
                            ...
                            <View style={item.sender_id === userId ? styles.sender : styles.receiver}>
                                <Text style={[styles.messageText, item.sender_id === userId ? styles.messageTextSender : styles.messageTextReceiver]}>{item.sender_id === userId ? 'You: ' : firstName + ' ' + lastName + ': '}{item.plaintext}</Text>
                                <Text style={[styles.messageTime, item.sender_id === userId ? styles.messageTimeSender : styles.messageTimeReceiver]}>{new Date(item.sent_at).toLocaleTimeString().split(':')[0].slice(0, 2) + ':' + new Date(item.sent_at).toLocaleTimeString().split(':')[1].slice(0, 2)}</Text>
                            </View>
                        </View>
                    )}
